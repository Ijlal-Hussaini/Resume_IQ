...
export const ExtractedResumeViewer: React.FC<ExtractedResumeViewerProps> = ({ resumeData }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <GlassCard className="p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.08] pb-5 mb-5">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-300">
            {contact.email && (
              <a href={`mailto:${contact.email}`} className="flex items-center gap-1.5 hover:text-violet-300 transition-colors">
            ...
            {contact.linkedin && (
              <a href={contact.linkedin} target="_blank" rel="noreferrer" className="flex items-center gap-1 hover:text-cyan-300">
                <Link className="w-3.5 h-3.5 text-cyan-400" />
                LinkedIn
              </a>
            )}
            {contact.github && (
              <a href={contact.github} target="_blank" rel="noreferrer" className="flex items-center gap-1 hover:text-slate-100">
      ...
      {/* Tabs */}
      <div className="flex bg-black/40 p-1.5 rounded-2xl border border-white/[0.06] overflow-x-auto">
        <button
          onClick={() => setActiveTab("experience")}
          className={`flex items-center gap-2 px-4 py-2 text-xs font-semibold rounded-xl whitespace-nowrap transition-all ${
            activeTab === "experience" ? "bg-violet-600 text-white shadow-lg" : "text-slate-400 hover:text-white"
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 text-xs font-semibold rounded-xl whitespace-nowrap transition-all ${
            activeTab === "skills" ? "bg-violet-600 text-white shadow-lg" : "text-slate-400 hover:text-white"
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 text-xs font-semibold rounded-xl whitespace-nowrap transition-all ${
            activeTab === "education" ? "bg-violet-600 text-white shadow-lg" : "text-slate-400 hover:text-white"
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 text-xs font-semibold rounded-xl whitespace-nowrap transition-all ${
            activeTab === "projects" ? "bg-violet-600 text-white shadow-lg" : "text-slate-400 hover:text-white"
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 text-xs font-semibold rounded-xl whitespace-nowrap transition-all ${
            activeTab === "raw" ? "bg-violet-600 text-white shadow-lg" : "text-slate-400 hover:text-white"
          }`}
        >
        ...
        <div className="space-y-4">
          {resumeData.work_experience.map((exp, idx) => (
            <GlassCard key={idx} className="p-5">
              ...
              <p className="text-xs text-violet-300 font-medium mb-3">
      ...
      {activeTab === "skills" && (
        <div className="space-y-4">
          {resumeData.skill_categories.map((cat, idx) => (
            <GlassCard key={idx} className="p-5">
              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-300 font-outfit mb-3">
                {cat.category_name}
              </h4>
        ...
        <div className="space-y-4">
          {resumeData.certifications.length > 0 && (
            <GlassCard className="p-5 border-amber-500/20">
              <h4 className="text-xs font-bold uppercase tracking-wider text-amber-300 font-outfit mb-3">
                Certifications & Licensure
              </h4>
              <div className="space-y-2.5">
                {resumeData.certifications.map((cert, idx) => (
                  <div key={idx} className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.06] text-xs text-slate-200">
      ...
      {activeTab === "raw" && (
        <GlassCard className="p-5">
          <pre className="p-4 rounded-xl bg-black/50 text-xs font-mono text-slate-300 whitespace-pre-wrap leading-relaxed max-h-[500px] overflow-y-auto">
